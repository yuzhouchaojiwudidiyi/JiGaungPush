...
//        mCircleSelectPaint.setStrokeWidth(mSelectCircleStroke);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.v("自定义view", "heightMode=" + heightMode);
        Log.v("自定义view", "widthSize=" + widthSize);
        Log.v("自定义view", "heightSize=" + heightSize);
        //保存测量结果
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mLineLength = getHeight() - getPaddingTop() - getPaddingBottom() - mCircleRadius * 2;
    }
    ...
    protected void onDraw(Canvas canvas) {  //文字在上面
        mCircleSelectPaint.setColor(mColorCircleDefault);
        mTextPaint.setColor(mColorCircleDefault);
        mLinePaint.setColor(mColorCircleDefault);
        int startTextY = mBounds.get(0).height(); //文字高度
        //绘制垂直线条
        canvas.drawLine(mCircleRadius, mCircleRadius, mCircleRadius, getHeight() - mCircleRadius, mLinePaint);
        //开是循环绘制view
        for (int i = 0; i < texts.size(); i++) {
            if (mSelectPosition >= i) {
                ...
                canvas.drawCircle(mCircleRadius, mCircleRadius + ((mLineLength / (texts.size() - 1)) * i), mCircleRadius, mCircleSelectPaint);
                Log.v("选择是那个", mSelectPosition + "     " + i);
                //画出线选择颜色
                if (mSelectPosition == i) { //新加
                    mLinePaint.setColor(mColorCircleSelect);
                    canvas.drawLine(mCircleRadius, mCircleRadius, mCircleRadius, mCircleRadius + ((mLineLength / (texts.size() - 1)) * i), mLinePaint);
                }
            ...
            //记录画圆的位置
            circleLoaction.add(new Float[]{Float.valueOf(mCircleRadius), mCircleRadius + ((mLineLength / (texts.size() - 1)) * i)});

            //绘制文字
            Log.i("wangjitao", "现在：" + startTextY);
            if (i == 0) {
                canvas.drawText(texts.get(i), mCircleRadius * 2 + mMarginTop, mCircleRadius + startTextY / 2, mTextPaint);
            } else if (i == texts.size() - 1) {
                canvas.drawText(texts.get(i), mCircleRadius * 2 + mMarginTop, getHeight() - mCircleRadius + startTextY / 2, mTextPaint);
            } else {
                canvas.drawText(texts.get(i), mCircleRadius * 2 + mMarginTop, mCircleRadius + ((mLineLength / (texts.size() - 1)) * i) + startTextY / 2, mTextPaint);
            }
        }
